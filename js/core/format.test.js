...
test('formatDotDateTime: ISO 日時をローカル暦日のドット区切りにする（ゼロ埋めあり）', () => {
  // TZ オフセット無しの ISO はローカル時刻として解釈されるため CI(UTC)/ローカル(JST) で不変
  assert.equal(formatDotDateTime('2026-06-26T09:05:00'), '2026.06.26');
  assert.equal(formatDotDateTime('2026-01-05T03:04:05'), '2026.01.05');
});

test('formatDotDateTime: 空文字・null は em dash を返す', () => {
  assert.equal(formatDotDateTime(''), '—');
  assert.equal(formatDotDateTime(null), '—');
  assert.equal(formatDotDateTime(undefined), '—');
});

test('formatDotDateTime: パースできない値は入力をそのまま返す', () => {
  assert.equal(formatDotDateTime('not-a-date'), 'not-a-date');
});
